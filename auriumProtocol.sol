...
contract AuriumToken is ERC20, ReentrancyGuard {
    ...
    */
    function transfer(
        ...
        uint256 FeeValueForComparison = value_ == 0 ? lastValue_.mul(calculatedBurnFee) : value_.mul(calculatedBurnFee);
        if (FeeValueForComparison > (_maxBurnFee.mul(PRECISION_FACTOR))) {
            calculatedBurnFee = value_ == 0 ? (_maxBurnFee.mul(PRECISION_FACTOR)).div(lastValue_) : (_maxBurnFee.mul(PRECISION_FACTOR)).div(value_);
...
/**
 * @title AuriumTokenFactory
 * @dev A factory contract for creating AuriumToken instances.
 */
contract AuriumTokenFactory is ReentrancyGuard {
    IERC20 public immutable paxGold_;
    using Strings for string;

    struct AuriumTokenData {
        address auriumTokenAddress;
        string name;
        string symbol;
        uint256 index;
    }

    uint256 internal auriumTokenCount_;
    uint256 internal immutable minPaxgDeposit_;
    mapping(uint256 => address) internal auriumTokenIndexToAddress_;
    mapping(address => AuriumTokenData) internal auriumTokenAddressToData_;
    mapping(string => uint) internal auriumTokenNameToIndex_;

    event AuriumTokenCreated(
        address indexed auriumTokenAddress,
        string name,
        string symbol
    );
    ...
    function createAuriumToken(
        ...
        require(
            auriumTokenNameToIndex_[tempName] == 0,
            "AuriumToken name already exists"
        );
        require(
            auriumTokenNameToIndex_[tempSymbol] == 0,
            "AuriumToken symbol already exists"
        );

        auriumTokenCount_++;

        AuriumToken auriumToken = new AuriumToken(
            address(paxGold_),
            _initialDeposit,
            _initialSupply,
            _name,
        ...
        auriumTokenAddressToData_[address(auriumToken)] = AuriumTokenData({
            auriumTokenAddress: address(auriumToken),
            name: _name,
            symbol: _symbol,
            index: auriumTokenCount_
        });

        require(
            paxGold_.transferFrom(
                msg.sender,
                address(auriumToken),
                _initialDeposit
            ),
        ...
        auriumTokenIndexToAddress_[auriumTokenCount_] = address(auriumToken);
        auriumTokenNameToIndex_[tempName] = auriumTokenCount_;
        auriumTokenNameToIndex_[tempSymbol] = auriumTokenCount_;

        emit AuriumTokenCreated(address(auriumToken), _name, _symbol);
    }
    ...
    /*
    @dev Gets the current AuriumToken count.
    @return auriumTokenCounts.
     */
    function getAuriumTokenCount()
        public
        view
        returns (uint256 auriumTokenCounts)
    {
        return auriumTokenCount_;
    }

    /*
    @dev Gets the AuriumToken address from index value.
    @param _num Index number
    @return AuriumTokenIndexToAddress - AuriumToken address.
     */
    function getAuriumTokenAddressFromIndex(
        uint256 _num
    ) public view returns (address AuriumTokenIndexToAddress) {
        return auriumTokenIndexToAddress_[_num];
    }

    /*
    @dev Gets the AuriumToken details from address.
    @param _addr Address of AuriumToken
    @return auriumTokenAddress - AuriumToken address.
    @return name - AuriumToken name.
    @return symbol - AuriumToken symbol.
     */
    function getAuriumTokenDataFromAddress(
        address _addr
    )
        public
        view
    ...
    {
        AuriumTokenData storage auriumTokenData = auriumTokenAddressToData_[
            _addr
        ];
        return (
            auriumTokenData.auriumTokenAddress,
            auriumTokenData.name,
            auriumTokenData.symbol,
            auriumTokenData.index
        );
    }
...
/**
 * @title AuriumTokenRouter
 * @dev A router contract for interacting with AuriumToken instances and
 * performing token swaps.
 */
contract AuriumTokenRouter is ReentrancyGuard {
    AuriumTokenFactory public immutable factory_;

    using SafeMath for uint256;

    /**
     * @dev Constructor to initialize the AuriumTokenRouter with the AuriumTokenFactory address.
     * @param _factory The address of the AuriumTokenFactory contract.
     */
    constructor(AuriumTokenFactory _factory) {
        factory_ = _factory;
    }

    /**
     * @dev Function to check if an AuriumToken contract is valid.
     * @param _atk The address of the AuriumToken contract.
     * @return A boolean indicating if the AuriumToken contract is valid.
     */
    function isValidATK(address _atk) public view returns (bool) {
        (, , , uint index) = factory_.getAuriumTokenDataFromAddress(_atk);
        return index != 0;
    }

    /**
     * @dev Function to get the AuriumToken balances of a user.
     * @param _user The address of the user.
     * @return AuriumTokenBalances - An array of the user's AuriumToken balances.
     */
    function getUserBalances(
        address _user
    ) public view returns (uint256[] memory AuriumTokenBalances) {
        uint256 auriumTokenCount = factory_.getAuriumTokenCount();
        uint256[] memory auriumTokenBalances = new uint256[](auriumTokenCount);
        for (uint256 i = 1; i < auriumTokenCount; i++) {
            address auriumTokenAddress = factory_
                .getAuriumTokenAddressFromIndex(i);
            AuriumToken auriumToken = AuriumToken(auriumTokenAddress);
            auriumTokenBalances[i] = auriumToken.balanceOf(_user);
        }
        return (auriumTokenBalances);
    }

    /**
     * @dev Function to swap one AuriumToken for another.
     * @param fromATK The address of the AuriumToken to swap from.
     * @param fromAmount The amount of fromATK to swap.
     * @param toATK The address of the AuriumToken to swap to.
     */
    function swapExactATKForATK(
        address fromATK,
        uint256 fromAmount,
        address toATK
    ) public nonReentrant {
        // Ensure that the fromATK and toATK are valid AuriumToken contracts
        require(
            isValidATK(fromATK),
            "fromATK is not a valid AuriumToken contract"
        );
        require(isValidATK(toATK), "toATK is not a valid AuriumToken contract");

        // Get the instances of the AuriumToken contracts
        AuriumToken fromToken = AuriumToken(fromATK);
        AuriumToken toToken = AuriumToken(toATK);
        ...
        // Ensure that the caller has sufficient balance of the from AuriumToken
        require(
            fromToken.balanceOf(msg.sender) >= fromAmount,
            "Insufficient from AuriumToken balance"
        );

        // Transfer the fromATK from the caller to the router, and approve the router to spend the fromAmount
        require(
            fromToken.transferFrom(msg.sender, address(this), fromAmount),
            "ATK Transfer to the router failed."
        );

        //calculate the received amount of from AuriumToken after deducting the burn fee.
        ...
        //Calculate the PaxgReceived to router contract after burning From AuriumToken
        ...
        // Calculate to AuriumTokens AuriumTokens Minted

        uint256 toAmount = toToken.getAuriumTokenAmountsOut(
        ...
        ); // send the AuriumToken that were recieved while mint.

        // Emit the Swap event
        emit Swap(msg.sender, fromAmount, fromATK, toATK, toAmount);
    }

    /**
     * @dev getSwapAmountsOut provides the desired token's value_ receivable to the caller.
     * @param _tokenA The address of the AuriumToken being swapped from.
     * @param _tokenB The amount of the AuriumToken being swapped to.
     * @param _amountA The amount of the AuriumToken being swapped from.
     */
    function getSwapAmountsOut(
        ...
        // Get the instances of the AuriumToken contracts
        AuriumToken fromToken = AuriumToken(_tokenA);
        AuriumToken toToken = AuriumToken(_tokenB);
     ...
     * @param amount The amount of the AuriumToken being swapped from.
     * @param fromATK The address of the AuriumToken being swapped from.
     * @param toATK The address of the AuriumToken being swapped to.
     * @param toAmount The amount of the AuriumToken being swapped to.
     */
    event Swap(
        address indexed user,
        uint256 amount,
        address indexed fromATK,
        address indexed toATK,
        uint256 toAmount
    );
}
