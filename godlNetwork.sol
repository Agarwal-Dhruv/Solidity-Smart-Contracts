    ...
    OwnableUpgradeable
{
     ...
     */
    function getToplineIndexByAddress(
        address _userAddress
    ) public view returns (uint256 toplineIndex) {
     ...
     */
    function getUserIdFromWalletAddress(
        address _userAddress
    ) public view returns (uint256 uniqueId) {
        require(
    ...
    function getUserReferralAddresses(
        address _userAddress,
        uint256 _level
    ) public view returns (address[] memory referralAddresses) {
        require(
            isExistingUser(_userAddress),
            "getUser: Given user has not yet joined the network."
        );
