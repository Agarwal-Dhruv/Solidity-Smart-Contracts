...
//     function setReinvestFlag(bool reinvest) external;

//     function buyLevelFor(address _for, uint256 level) external;

// function getUser(address user) external view returns (User memory);

//     function getDepositTiers() external view returns (uint256[] memory);
...
//     function getTemporaryBalance(address user) external view returns (uint256);

//     function getReinvestFlag(address user) external view returns (bool);
// }
...
contract GodlMLM is PausableUpgradeable, OwnableUpgradeable {
    ...
    function initialize(
        ...
        godlToken = GODL(godlTokenAddress);
    }

    function join(address referrer) external whenNotPaused {
        require(referrer != address(0), "Referrer address is not valid");
        require(
        ...
        uint256 currentReferralLevel = users[referrer].referralCount[0];
        users[referrer].referrals[currentReferralLevel].push(msg.sender);
        users[referrer].referralCount[0]++;

        emit NewUser(msg.sender, referrer);
    }

    function setReinvestFlag(bool reinvest) external whenNotPaused {
    ...
    function buyLevelFor(address _for, uint256 level) external whenNotPaused {
        require(_for != address(0), "Invalid address");
        require(level > 0 && level <= depositTiers.length, "Invalid level");
        require(users[_for].referrer != address(0), "User not joined");
        require(
        ...
        console.log("amountToBeSent",amount);
        console.log("UserBalance", paxgToken.balanceOf(msg.sender));
        paxgToken.safeTransferFrom(msg.sender, address(this), amount);

        users[_for].level = level;
        emit LevelUp(_for, level);

        _mintGodl(_for, amount);
        _distributeRewards(_for, amount);
    ...
    function _mintGodl(address to, uint256 paxgAmount) private whenNotPaused {
        // Mint GODL tokens for the user
        godlToken.mint(to, paxgAmount);

        // Use the transfer function of the GODL contract to trigger the deflationary mechanism
        IERC20(address(godlToken)).transfer(to, paxgAmount);
    }
