...
contract StakeInitializable is Ownable, ReentrancyGuard {
    ...
    function deposit(
        ...
        x = user.depositTime = block.timestamp;
        y = user.withdrawTime = user.depositTime + (period_ * 24 * 60 * 60);
        z = pool.totalDeposits = pool.totalDeposits + user.amount;
...
contract StakeTokenFactory is Ownable {
    ...
    address[] public stakingPrograms;
    // Maps staking program address with its details.
    mapping(address => StakingProgramInfo) public _stakingProgramInfo;

    event NewStakingContract(address indexed stakingContract);
     
    uint256 public stakingProgramsDeployed;
    ...
    function initializeStakingContract(
        ...
        StakeInitializable(stakeInitializableAddress).initialize(
            stakedToken_,
            rewardToken_,
            rewardPerSecond_,
            startTime_,
        ...
        _stakingProgramInfo[
            address(stakeInitializableAddress)
        ] = StakingProgramInfo({
            ...
            endTime: endTime_,
            timePeriods: timePeriods_,
            penalties: penalties_,
            fixedAPYs: fixedAPYs_,
            admin: admin_
        });

        stakingProgramsDeployed++;
